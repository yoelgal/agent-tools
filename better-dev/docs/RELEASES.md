...
0.9.7 reonboard,offer - graphify setup stops re-pointing git's global excludes: step 2 now reads core.excludesFile in every scope (system, global, local), appends to the file git actually reads, refuses a relative or ~user value instead of writing into the host repo, reports a failed append as failed, and sets the key only where a probe shows git does not already reach that file; the graphifyy install is pinned to >=0.9.18 against pypi.org, the upgrade announce is the observed version change rather than uv's exit code (uv exits 0 on a no-op), the bare-repo seed source is the default-branch worktree rather than whichever peer git listed first, and D26 lists every machine-global write with its undo. OFFER: a machine set up before this release may be mis-pointed - run `git config --global --get core.excludesfile`, and if it names a file whose only line is `graphify-out/`, the operator's real global ignore has been inert in every repo since; `git config --global --unset core.excludesfile` restores it, then re-run /graphify-wrapper-setup to re-add the line where it belongs
